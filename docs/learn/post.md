...
#### Why Require Proof of Work?

However, even with the nonce requirement, a smesher with access to a powerful machine can try a large number of nonces in parallel and thus find enough labels below the threshold without allocating the complete required disk space. As such, someone with an ASIC can try 5000 nonces in parallel but allocate, for example, 90% of the disk space required to find the target number of nonces. Thus, even with a nonce value, the smesher with the powerful machine gets an advantage. Hence, there needed to be some way to eliminate this advantage that a smesher with a powerful machine would have over other smeshers.

The solution to this problem was to introduce the requirement of doing some computation (work) for each new nonce value generated. The work here entails hashing several values together in order to get the nonce value. The **K2PoW** algorithm (which itself relies on a proof-of-work
algorithm called [RandomX](https://github.com/tevador/RandomX)) is used to perform this task: the challenge value, nonce value, and the smesher ID are hashed together which results in the nonce value. Doing this work would mean that every smesher would have to perform it whenever they try a new nonce value. While doing this work would not hinder the general smesher (who would only see a spike in CPU usage for a few moments), someone trying to paralellize nonce trials would end up having to spend a lot of computational resources, thereby evaporating any advantage they may have otherwise gotten.
