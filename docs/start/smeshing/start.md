...
2. **[Proof of Elapsed Time (PoET)](../../learn/post.md/#proof-of-elapsed-time)**: In this step, your node automatically submits the initial PoS value to a specialized [PoET server](./smeshing_adv/poet.md), which then uses it to generate the PoET over a period of time called a "PoET round". This period is equal in length to an epoch (two weeks) but does not necessarily overlap it completely. The PoET value can then be retrieved by your node at the end of a PoET round.
