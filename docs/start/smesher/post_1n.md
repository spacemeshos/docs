...
2. Rename your source node's `key.bin` file to a more descriptive name, such as `my_super_secret_key.key`. This naming convention is for your convenience only. Ensure that on the target node, there is no file named `key.bin`.
